...
<script type="text/javascript">
	...
	function delQuestion(id){
		...
		var action="<%=basePath%>/question/deleteQuestion";
		parent.showConfirm("你确定删除吗？","questionForm",action);
...
<body>
	<form action="manaQuestion" class="uk-form" name="pageForm" id="questionForm">
		...
		<fieldset data-uk-margin >
				<div class="uk-form-select uk-button uk-active uk-width-1-10" data-uk-form-select>
					...
					<option value="1" <c:if test="${question.state == 1}">selected</c:if> >启用</option>
					<option value="0" <c:if test="${question.state == 0}">selected</c:if> >禁用</option>
				</select>
				</div>
				<input
				class="uk-margin-small-top" type="text" placeholder="标题关键字"
				name="name" value="${question.name }">
			<button class="uk-button">搜索</button>
			<a href="createQuestion" class="uk-icon-justify uk-icon-plus uk-icon-medium"></a>
		</fieldset>
		...
		<table class="uk-table .uk-table-hover .uk-table-striped">
			...
			<tbody>
				<c:forEach var="question" items="${questions }" varStatus="c">
					<tr>
						<td>${c.index+1 }</td>
						<td>${question.name }</td>
						<td id="abc">
						<c:if test="${question.state==0 }">禁用</c:if>
						<c:if test="${question.state==1 }">启用</c:if>
						</td>
						<td><fmt:formatDate value="${question.optime }"
								pattern="yyyy-MM-dd HH:mm:ss" /></td>
						<td>
							<div class="uk-button-group">
								<a class="uk-button uk-button-primary" href="editQuestion?questionid=${question.id }">编辑</a>
								<a class="uk-button uk-button-primary" href="#" onclick="disableQuestion(${question.id },this)">
									<c:if test="${question.state==0 }">启用</c:if>
									<c:if test="${question.state==1 || (question.state!=0 && question.state!=1)}">禁用</c:if>
								</a>
								<a class="uk-button uk-button-danger" href="#" onclick="delQuestion(${question.id })">删除</a>
							</div>
						</td>
					</tr>
				</c:forEach>
